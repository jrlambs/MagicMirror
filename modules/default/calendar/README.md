...
| `timeFormat`                 | Display event times as absolute dates, or relative time, or absolute date headers with times listed for each event <br><br> **Possible values:** `absolute` or `relative` or `dateheaders` <br> **Default value:** `relative`
